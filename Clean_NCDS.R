...
###############################################################################
# IOTF BMI cutoffs (Cole et al. 2000)
###############################################################################

ages <- seq(2, 18, by = 0.5)

overweight_male <- c(18.41, 18.13, 17.89, 17.69, 17.55, 17.47, 17.42, 17.45, 17.55, 17.71, 17.92, 18.16, 18.44, 18.76, 19.10,
                     19.46, 19.84, 20.20, 20.55, 20.89, 21.22, 21.56, 21.91, 22.27, 22.62, 22.96, 23.29, 23.60, 23.90, 24.19,
                     24.46, 24.73, 25.00)

obese_male <- c(20.09, 19.80, 19.57, 19.39, 19.29, 19.26, 19.30, 19.47, 19.78, 20.23, 20.63, 21.09, 21.60, 22.17, 22.77,
                23.39, 24.00, 24.57, 25.10, 25.58, 26.02, 26.43, 26.84, 27.25, 27.63, 27.98, 28.30, 28.60, 28.88, 29.14,
                29.41, 29.70, 30.00)

overweight_female <- c(18.02, 17.76, 17.56, 17.40, 17.28, 17.19, 17.15, 17.20, 17.34, 17.53, 17.75, 18.03, 18.35, 18.69, 19.07,
                       19.45, 19.86, 20.29, 20.74, 21.20, 21.68, 22.14, 22.58, 22.98, 23.34, 23.66, 23.94, 24.17, 24.37, 24.54,
                       24.70, 24.85, 25.00)

obese_female <- c(19.81, 19.55, 19.36, 19.23, 19.15, 19.12, 19.17, 19.34, 19.65, 20.08, 20.51, 21.01, 21.57, 22.18, 22.81,
                  23.46, 24.11, 24.77, 25.42, 26.05, 26.67, 27.24, 27.76, 28.20, 28.57, 28.87, 29.11, 29.29, 29.43, 29.56,
                  29.69, 29.84, 30.00)

male_table <- tibble(
  age = ages,
  sex = "M",
  overweight = overweight_male,
  obese = obese_male
)
...
female_table <- tibble(
  age = ages,
  sex = "F",
  overweight = overweight_female,
  obese = obese_female
)

iotf_cutoffs <- bind_rows(male_table, female_table)
